...
if __name__ == '__main__':
    ...
    if CHECK_FILES_ARE_NOT_CORRUPT:
        from MWTracker.analysis.compress.compressVideo import selectVideoReader
        
        corrupt_videos = []
        for ii in range(len(full_movie_files)):
            mf = full_movie_files[ii]
            print(ii, len(full_movie_files))
            
            try:
                vid = selectVideoReader(mf)
            except OSError:
